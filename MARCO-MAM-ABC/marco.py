...
def parse_args():
    ...
    exp_group.add_argument('--rnd-init', type=int, nargs='?', const=1, default=None,
                           # default = val if --rnd-init not specified; const = val if --rnd-init specified w/o a value
...
def setup_csolver(args, seed):
    ...
    if args.cnf or infile.name.endswith('.cnf') or infile.name.endswith('.cnf.gz') or infile.name.endswith(
        ...
        try:
            ...
            error_exit("Unable to use MUSer2 for MUS extraction.",
                       "Use --force-minisat to use Minisat instead (NOTE: it will be much slower.)", e)
        except (IOError, OSError) as e:
            error_exit("Unable to load pyminisolvers library.", "Run 'make -C pyminisolvers' to compile the library.",
                       e)
        infile.close()
...
def print_result(result, args, stats, num_constraints):
    ...
    output = result[0]
    if args.alltimes:
        output = "%s %0.8f" % (output, stats.total_time())
    if args.verbose:
        #output = "%s %s" % (output, " ".join([str(x) for x in result[1]]))

        output = "%s " % (output)  # %s, " ".join([str(x) for x in result[1]])
    # if result[0]=='U':
    print(output)
