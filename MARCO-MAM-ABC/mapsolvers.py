...
class MapSolver(object):
    ...
    def __init__(self, n, bias=True, dump=None):
        ...
        self.list = []  #
        self.mid_seed = None  #added by jly
        self.count = 0
        self.n = n
        self.bias = bias
    ...
    def get_twoseed(self):

        seed = self._solver.get_model_trues(start=0, end=self.n, offset=1)

        # self._solver.block_seed(seed)
        k = len(seed)
        ...
        while True:
            # print' seed is:',seed,self.complement(seed)
            # my_seed=list(self.complement(seed))#random.sample(seed,s)
            my_seed = random.sample(seed, s)
            # print 'my_seed is:',my_seed
            sd = self._solver.check_complete(positive_lits=my_seed)
            if sd:
                # print 'check_seed return :',sd
                break
            else:
                return seed, None
        # print 'sss'
    ...
    def get_seed(self):
        ...
        if(s>1):#k>s
              ...
              return seed
        """
        # print'list[0] is:',self.list[0]
        # if  self.list[0] is None: #if list is empty
        theseed, self.mid_seed = self.get_twoseed()
        #self.list.append(seed1)

        # if seed2 is not None:
        # if self._solver.check_complete(positive_lits=seed2):
	#-------------------------------------------------------------------revised-------------------------------------
	"""
		if seed2 is not None:
		if self._solver.check_complete(positive_lits=seed2):
        		self.list.append(seed2)
	#-------------------------------------------------------------------revise done-------------------------------------"""
        #theseed = self.list[0]
        # print'theseed is:',theseed
        # self._solver.block_seed(theseed)
    ...
    # seed=[i+1 for i in range(self.n) if model[i]]
...
class MinicardMapSolver(MapSolver):
    def __init__(self, n, bias=True,
                 rand_seed=None):  # bias=True is a high/inclusion/MUS bias; False is a low/exclusion/MSS bias.
    ...
    def solve_with_bound(self, k):
        ...
        return self._solver.solve(
            [-(self.n + x + 1) for x in range(k)] + [(self.n + k + x + 1) for x in range(self.n - k)])
...
class MinisatMapSolver(MapSolver):
    def __init__(self, n, bias=True, rand_seed=None,
    ...
    def next_seed(self):
        if self._solver.solve():
            """seed1,seed2=self.get_seed()
            self.list.append(seed1)
            if seed2 is not None:
            self.list.append(seed2)
            nextseed = self.list[0]
               del self.list[0]
            return nextseed
            """
