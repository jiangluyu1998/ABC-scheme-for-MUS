    ...
    def enumerate(self):
        ...
        for seed, known_max in self.seeds:
            ...
            with self.stats.time('check'):
                ...
                seed_is_sat = self.subs.check_subset(seed, improve_seed=False)
            ...
            if seed_is_sat:
                ...
                if known_max:
                    MSS = seed
                    if len(MSS) == self.n-1:
                        num=list(self.subs.complement(MSS))
                        #print("num:")
                        #print(num)
                        submus.append(num[0])
                        #print("submus:")
                        #print(submus)
                        self.map.block_down(submus)
                    #print(" seed is maximal ,it is an MSS")
                else:
                    #print("seed is not maximal, grow it to MSS")
                ...
                with self.stats.time('block'):
                    res = ("S", MSS)
                    mss_num=mss_num+1
                    print("MUS number is %d " % mus_num)
                    print("MSS number is %d " % mss_num)
                    yield res
                ...
                midseed = self.map.mid_seed
                #print("- midseed: %s" % " ".join([str(x) for x in midseed]))
                if midseed is not None:
                    #print("- midseed: %s" % " ".join([str(x) for x in midseed]))
                    if self.map._solver.check_complete(positive_lits=midseed):
                        #print("mid seed is unexplored")
                        midseed_is_sat = self.subs.check_subset(midseed, improve_seed=False)
                        if midseed_is_sat:
                            #print("midseed is sat")
                            # grow midseed and blockdown

                            with self.stats.time('grow'):
                                oldlen = len(midseed)
                                MSS = self.subs.grow(midseed)
                                self.record_delta('grow', oldlen, len(MSS), True)
                            ...
                            #if self.config['verbose']:
                            #print("- MSS blocked.")
                        else:  # midseed is unsat
                            #shrink midseed and blockup
                            #print("midseed is unsat")
                            with self.stats.time('shrink'):
                                oldlen = len(midseed)
                                MUS = self.subs.shrink(midseed)
                                #print("shrink midseed returned MUS:")
                                #print(MUS)
                                """
                                if MUS is None:
                                    # seed was explored in another process
