...
class Solver(object):
    ...
    def block_seed(self, seed):  # my change
        """Block the current seed in order to avoid to be found again"""
        self.add_clause([-(x + 1) if seed[x] > 0 else x + 1 for x in range(1, len(seed))])
...
class SubsetMixin(Solver):
    ...
    def unsat_core(self, offset=0):  # type: (int) -> array.array
        ...
        if self._origvars is None:
            raise Exception(
                "SubsetSolver.set_varcounts() must be called (and at least one instrumented constraint added) before .unsat_core()")
...
class MinicardSubsetSolver(SubsetMixin, MinicardSolver):
    ...
    def add_atmost_instrumented(self, lits, k, index):  # type: (Sequence[int], int, int) -> None
        ...
        if self._origvars + 1 + index > self.nvars():
            raise Exception("Relaxation variable %i has not been created yet.  Call new_var() first." % (
                    self._origvars + 1 + index))
