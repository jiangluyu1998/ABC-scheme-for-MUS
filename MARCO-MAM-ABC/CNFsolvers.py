...
class MinisatSubsetSolver(object):
    ...
    def check_subset(self, seed, improve_seed=False):
        ...
        '''
        global is_sat1  # charge whther the middle-subset is SAT
        if  is_sat == False:
            k=len(seed)
            if(self.n %2):
                so=self.n/2+1 
                ...
                so=self.n/2
           # print 'so is:',so,'  k is:  ',k# so is the middle of n contrains
            if(k>so):  #Upper half
             
                seed1= random.sample(seed,so)
            #print'seed1 is:  ',seed1
            s=[i-1 for i in seed1]
            is_sat1 = self.s.solve_subset([i-1 for i in seed1])
            #print'the seed1 is:   ',is_sat1
    
            if (is_sat != is_sat1):
                 is_sat = self.s.solve_subset([i-1 for i in seed])#how to grow up to find mss
            else:
                 is_sat = self.s.solve_subset([i-1 for i in seed1])
                 #print'is_sat== is_sat1'
        else:
            is_sat1=False
        #print'@.....@ is_sat is:',is_sat
        '''
...
class MUSerSubsetSolver(MinisatSubsetSolver):
    ...
    def shrink(self, seed1):
        #print("in the shrink2 function,seed is:")
        #print(seed1)
        ...
        if not self._msolver.check_seed(seed1):
            return None
        ...
        if len(seed1) == len(hard):
            return seed1

        # Open tmpfile
        with tempfile.NamedTemporaryFile('wb') as cnf:
            self.write_CNF(cnf, seed1, hard)
        ...
        ret = [seed1[int(x) - 1] for x in matchline.split()[1:-1] if int(x) > 0]

        # Add back in hard clauses
        ret.extend(hard)

        assert len(ret) <= len(seed1)
...
class ImprovedImpliesSubsetSolver(MinisatSubsetSolver):
    ...
    def grow(self, seed):
        ...
        return current
        def getMUSMSSnum(self):
            return self._known_MUS, self._known_MSS
